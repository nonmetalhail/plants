export const cacti = [
  {
    binomialName: 'Austrocylindropuntia subulata',
    commonNames: ["Eve's needles"],
    links: [
      'https://en.wikipedia.org/wiki/Austrocylindropuntia_subulata',
      'https://cactusjungle.com/plants/austrocylindropuntia-subulata/'
    ]
  }, {
    binomialName: 'Cephalocereus senilis',
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/6893/Cephalocereus_senilis']
  }, {
    binomialName: 'Cereus aethiops',
    confidence: 2,
    links: [
      'http://www.cactusinhabitat.org/index.php?p=specie&id=14',
      'https://worldofsucculents.com/cereus-aethiops/',
      'https://cactiguide.com/cactus/?genus=Cereus&species=aethiops'
    ]
  }, {
    binomialName: 'Cereus forbesii cv. Spiralis',
    commonNames: ['Spiral cactus']
  }, {
    binomialName: 'Cereus forbesii fa. monstrose "Ming Thing"',
    links: ['https://cactusjungle.com/plants/cereus-monstrose-ming-thing/'],
    synonyms: ['C. validus']
  }, {
    binomialName: 'Cereus hildmannianus f. cristata monstruosa',
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/28484/Cereus_hildmannianus_f._cristata_monstruosa']
  }, {
    binomialName: 'Cereus repandus f. spineless',
    commonNames: ['Spineless apple cactus'],
    synonyms: ['Cereus peruvianus'],
    confidence: 2
  }, {
    binomialName: 'Cereus repandus f. monstrose',
    commonNames: ['Peruvian apple cactus']
  }, {
    binomialName: 'Cereus repandus f. monstrose "Rojo"',
    links: [
      'https://cactusjungle.com/plants/cereus-monstrose-rojo/',
      'https://thebelmontrooster.com/families-of-familiar-plants/cactaceae-family/cereus-repandus-monstruosus-cv-rojo-syn-cereus-peruvianus-monstrose-cv-rojo/'
    ]
  }, {
    binomialName: 'Cereus spegazzinii f. cristatus',
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/15183/Cereus_spegazzinii_f._cristatus']
  }, {
    binomialName: 'Cleistocactus strausii',
    commonNames: ['Silver torch'],
    synonyms: ['Borzicactus strausii'],
    links: ['https://en.wikipedia.org/wiki/Cleistocactus_strausii']
  }, {
    binomialName: 'Cleistocactus winteri',
    commonNames: ['Golden Rat Tail'],
    synonyms: ['Borzicactus winteri'],
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/3784/Cleistocactus_winteri']
  }, {
    binomialName: 'Cylindropuntia sp.1',
    commonNames: ['Cholla']
  }, {
    binomialName: 'Cylindropuntia sp.2',
    commonNames: ['Cholla']
  }, {
    binomialName: 'Disocactus speciosus',
    confidence: 3
  }, {
    binomialName: 'Disocactus × mallisonii',
    links: ['https://worldofsucculents.com/disocactus-mallisonii-rats-tail-cactus/'],
    note: 'Horticultural hybrid beetween Disocactus flagelliformis and Disocactus speciosus'
  }, {
    binomialName: 'Echinocactus grusonii',
    commonNames: ['Golden Barrel'],
    links: [
      'http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/3345/Echinocactus_grusonii',
      'https://en.wikipedia.org/wiki/Echinocactus_grusonii'
    ]
  }, {
    binomialName: 'Echinocactus polycephalus',
    links:[
      'http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/940/Echinocactus_polycephalus',
      'https://www.birdandhike.com/Veg/Species/Cactus/Echino_pol/_Ech_pol.htm'
    ]
  }, {
    binomialName: 'Echinocereus engelmannii',
    commonNames: ["Engelmann's Hedgehog"],
    links: ['https://www.birdandhike.com/Veg/Species/Cactus/Echino_eng/_Ech_eng.htm']
  }, {
    binomialName: 'Echinocereus pulchellus subs. weinbergii',
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/12900/Echinocereus_pulchellus_subs._weinbergii']
  }, {
    binomialName: 'Echinocereus reichenbachii f. albispinus'
  }, {
    binomialName: 'Echinocereus rigidissimus subs. rubispinus',
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/10573/Echinocereus_rigidissimus_subs._rubispinus']
  }, {
    binomialName: 'Echinopsis candicans f. cristata',
    confidence: 2,
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/18454/Echinopsis_candicans_f._cristata']
  }, {
    binomialName: 'Echinopsis lageniformis f. Monstruosa',
    synonyms: ['Trichocereus bridgesii f. monstruosus'],
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/18442/Echinopsis_lageniformis_f._monstruosa_(Short_joined_type)']
  }, {
    binomialName: 'Echinopsis pachanoi',
    commonNames: ['San Pedro Cactus'],
    synonyms: ['Trichocereus pachanoi'],
    links: [
      'https://www.gardenia.net/plant/trichocereus-pachanoi',
      'https://en.wikipedia.org/wiki/Echinopsis_pachanoi'
    ]
  }, {
    binomialName: 'Echinopsis schickendantzii',
    confidence: 2,
    links: [
      'https://cactiguide.com/cactus/?genus=Echinopsis&species=schickendantzii',
      'http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/16016/Echinopsis_schickendantzii'
    ]
  }, {
    binomialName: 'Echinopsis spachiana',
    links: [
      'http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/8648/Echinopsis_spachiana',
      'https://cactiguide.com/cactus/?genus=Echinopsis&species=spachiana'
    ]
  }, {
    binomialName: 'Echinopsis terscheckii',
    synonyms: ['Trichocereus terscheckii'],
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/8744/Echinopsis_terscheckii']
  }, {
    binomialName: 'Echinopsis x ?',
  }, {
    binomialName: 'Epiphyllum sp.1',
  }, {
    binomialName: 'Epiphyllum sp.2',
  }, {
    binomialName: 'Espostoa lanata',
    commonNames: ['Old man cactus'],
    links: ['https://en.wikipedia.org/wiki/Espostoa_lanata']
  }, {
    binomialName: 'Ferocactus cylindraceus',
    commonNames: ['California barrel'],
    links: [
      'http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/10102/Ferocactus_cylindraceus',
      'https://birdandhike.com/Veg/Species/Cactus/Feroca_cyl/_Fer_cyl.htm'
    ]
  }, {
    binomialName: 'Ferocactus emoryi subs. rectispinus',
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/21497/Ferocactus_emoryi_subs._rectispinus']
  }, {
    binomialName: 'Ferocactus glaucescens',
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/1532/Ferocactus_glaucescens']
  }, {
    binomialName: 'Ferocactus latispinus',
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/4259/Ferocactus_latispinus'],
    dead: true
  } ,{
    binomialName: 'Ferocactus latispinus subs. spiralis',
    confidence: 2,
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/4281/Ferocactus_latispinus_subs._spiralis']
  }, {
    binomialName: 'Ferocactus peninsulae',
    confidence: 2,
    links: [
      'http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/11990/Ferocactus_peninsulae_subs._townsendianus',
      'https://cactiguide.com/cactus/?genus=Ferocactus&species=peninsulae'
    ]
  }, {
    binomialName: 'Ferocactus pringlei',
    commonNames: ['Mexican Fire Barrel'],
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/13434/Ferocactus_pringlei']
  }, {
    binomialName: 'Ferocactus stainesii',
    commonNames: ['Mexican Fire Barrel'],
    links: [
      'http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/13428/Ferocactus_stainesii',
      'https://en.wikipedia.org/wiki/Ferocactus_pilosus'
    ]
  }, {
    binomialName: 'Ferocactus sp.',
    note: 'From seeds from Terry'
  }, {
    binomialName: 'Leuchtenbergia principis',
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/12924/Leuchtenbergia_principis']
  }, {
    binomialName: 'Lophocereus marginatus',
    commonNames: ['Mexican Fencepost'],
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/8046/Lophocereus_marginatus']
  }, {
    binomialName: 'Mammillaria backebergiana',
    confidence: 3,
    alternate: ['M. yucatanensis'],
    links: ['https://identify.plantnet.org/the-plant-list/species/Mammillaria%20backebergiana%20F.G.%20Buchenau/data']
  }, {
    binomialName: 'Mammillaria decipiens',
    confidence: 2
  }, {
    binomialName: 'Mammillaria magnifica',
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/30361/Mammillaria_magnifica']
  }, {
    binomialName: 'Mammillaria muehlenpfordtii',
    synonyms: ['M. celsiana'],
    confidence: 2,
    links: ['https://thebelmontrooster.com/families-of-familiar-plants/cactaceae-family/mammillaria-species/mammillaria-muehlenpfordtii-syn-m-celsiana-golden-pincushion/']
  }, {
    binomialName: 'Mammillaria polythele subs. obconella',
    confidence: 3
  }, {
    binomialName: 'Mammillaria rhodantha subs. pringlei',
    confidence: 2,
    commonNames: ['Lemon ball cactus', 'Rainbow pincushion'],
    links: [
      'https://cactusjungle.com/plants/mammillaria-rhodantha-ssp-pringlei/',
      'http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/14525/Mammillaria_rhodantha_subs._pringlei'
    ]
  }, {
    binomialName: 'Mammillaria senilis',
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/9677/Mammillaria_senilis'],
  }, {
    binomialName: 'Mammillaria spinosissima',
    commonNames: ['Red headed irishman'],
    links: ['https://en.wikipedia.org/wiki/Mammillaria_spinosissima']
  }, {
    binomialName: 'Mammillaria vetula subs. gracilis',
    confidence: 2,
    synonyms: ['M. gracilis fragilis'],
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/9166/Mammillaria_vetula_subs._gracilis']
  }, {
    binomialName: 'Mammillaria sp',
    confidence: 3
  }, {
    binomialName: 'Myrtillocactus geometrizans',
    commonNames: ['Whortleberry cactus'],
    links: ['https://en.wikipedia.org/wiki/Myrtillocactus_geometrizans']
  }, {
    binomialName: 'Opuntia monacantha v. Maverick variegata',
    commonNames: ["Joseph's coat"],
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/19873/Opuntia_monacantha_var._variegata_cv._Maverick_variegata']
  }, {
    binomialName: 'Oreocereus trollii',
    commonNames: ['Old man of the Andes']
  }, {
    binomialName: 'Pachycereus pringlei',
    commonNames: ['Cardon', 'Mexican giant cardon'],
    links: ['https://en.wikipedia.org/wiki/Pachycereus_pringlei']
  }, {
    binomialName: 'Parodia warasii',
    synonyms: ['Notocactus warasii'],
    links: [
      'http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/11868/Notocactus_warasii',
      'https://en.wikipedia.org/wiki/Parodia_warasii'
    ]
  }, {
    binomialName: 'Parodia schumanniana',
    confidence: 2,
    links: [
      'https://cactiguide.com/cactus/?genus=Parodia&species=schumanniana',
      'http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/1141/Parodia_schumanniana'
    ]
  }, {
    binomialName: 'Parodia leninghausii',
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/14697/Parodia_leninghausii']
  }, {
    binomialName: 'Parodia erubescens',
    confidence: 2,
    links: [
      'https://identify.plantnet.org/the-plant-list/species/Parodia%20erubescens%20(Osten)%20D.R.%20Hunt/data',
      'https://cactusjungle.com/plants/parodia-erubescens/'
    ]
  }, {
    binomialName: 'Parodia magnifica',
    confidence: 2,
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/20453/Parodia_magnifica']
  }, {
    binomialName: 'Pilosocereus pachycladus',
    confidence: 2,
    commonNames: ['Bright wooley blue spire'],
    links: [
      'https://cactusjungle.com/plants/pilosocereus-pachycladus/',
      'http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/16443/Pilosocereus_pachycladus'
    ]
  }, {
    binomialName: 'Pilosocereus azureus',
    confidence: 2,
    commonNames: ['Wooley blue spires'],
    links: ['http://www.llifle.com/Encyclopedia/CACTI/Family/Cactaceae/16452/Pilosocereus_azureus']
  }, {
    binomialName: 'Tephrocactus articulatus var. papyracanthus'
  }
];